...
const parseTradeDate = (trade: Trade) => getTradeDate(trade)
...
const buildCalendarRows = (month: MonthOption | undefined, trades: Trade[]): CalendarCellWithTrades[][] => {
  ...
  const startingOffset = (firstOfMonth.getDay() + 6) % 7 // convert Sunday=0 to Monday=0
  ...
  for (let day = 1; day <= daysInMonth; day += 1) {
    ...
    const fullDate = new Date(month.year, month.month, day)
    buffer.push({
...
export function CalendarPage({ userId, selectedAccounts = null }: Props) {
  ...
  const fallbackMonth = useMemo<MonthOption>(() => {
    ...
    return { month, year, label: monthFormatter.format(new Date(year, month, 1)) }
  }, [])
  ...
  const monthTrades = useMemo(() => {
    ...
    const startMs = new Date(activeMonth.year, activeMonth.month, 1).getTime()
    const endMs = new Date(activeMonth.year, activeMonth.month + 1, 1).getTime()
  ...
  const activeMonthKey = activeMonth ? `${activeMonth.year}-${activeMonth.month}` : ''
  const visibleHoverInfo = hoverInfo?.monthKey === activeMonthKey ? hoverInfo : null
      ...
      <section className="panel calendar">
        ...
        <div className="calendar-controls">
          <div className="chip-row">
            <button
              ...
              onClick={() => {
                closeHoverInfo()
                setMonthCursor((idx) => Math.min(idx + 1, months.length - 1))
              }}
              disabled={!canGoPrev}
            >
              ‹
            ...
            <button
              ...
              onClick={() => {
                closeHoverInfo()
                setMonthCursor((idx) => Math.max(idx - 1, 0))
              }}
              disabled={!canGoNext}
            >
              ›
              ...
              {row.map((cell, cellIdx) => (
                <div
                  ...
                  onClick={() => {
                    ...
                    closeHoverInfo()
                    navigate(`/journal?date=${formatLocalDate(cell.fullDate)}`)
                  }}
                >
        ...
        {visibleHoverInfo && (
          <div
            ...
            style={{ top: visibleHoverInfo.position.top, left: visibleHoverInfo.position.left }}
            onMouseEnter={clearHideTimeout}
            onMouseLeave={scheduleHide}
          >
            <div className="tooltip-header">{visibleHoverInfo.label}</div>
            <div className="tooltip-table">
              <div className="tooltip-row head">
                <span>Ticker</span>
                <span>PnL</span>
              </div>
              {visibleHoverInfo.trades.map((trade, idx) => (
